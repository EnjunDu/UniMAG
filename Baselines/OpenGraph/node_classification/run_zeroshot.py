...
# from sklearn.metrics import accuracy_score, f1_score  # 版本兼容性问题
...
class ZeroShotInference:
    def __init__(self, model_path, data_name='books-nc'):
...
def main():
    ...
    for i, arg in enumerate(sys.argv[1:]):
        if arg == '--model_path' and i + 1 < len(sys.argv):
            model_path = sys.argv[i + 2]
        elif arg == '--data_name' and i + 1 < len(sys.argv):
            data_name = sys.argv[i + 2]
    ...
    # 运行zero-shot推理
    inference = ZeroShotInference(model_path, data_name)
