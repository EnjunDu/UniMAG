...
import pickle
import os
# import setproctitle  # 可选依赖，已注释
import time
# from sklearn.metrics import f1_score  # 版本兼容性问题，已注释

def f1_score(y_true, y_pred, average='macro'):
    """简单的F1分数计算函数，替代sklearn版本"""
...
class Exp:
    ...
    def prepare_model(self):
        ...
        if args.multi_gpu and t.cuda.device_count() > 1:
            print(f"使用 {t.cuda.device_count()} 个GPU")
            self.model = t.nn.DataParallel(self.model)
        
        # 移动到GPU
        self.model = self.model.to(args.devices[1])
        ...
        if args.mixed_precision:
            try:
                ...
                print("警告: 无法导入混合精度模块，使用标准精度")
                args.mixed_precision = 0
    ...
    def test_epoch(self, tst_loader, tst_handler):
        with t.no_grad():
            ...
            # 分块处理大型数据集
            chunk_size = args.chunk_size if ep_tstnum > args.chunk_size else ep_tstnum
            ...
            for i, batch_data in enumerate(tst_loader):
                ...
                # 使用混合精度进行推理
                if args.mixed_precision and hasattr(self, 'autocast'):
                    with self.autocast():
    ...
    def load_model(self):
        ...
        try:
            ...
            log('Warning: Could not load history file')
            self.metrics = {}
        log('Model Loaded')
...
if __name__ == '__main__':
    ...
    # setproctitle.setproctitle('OpenGraph')  # 可选功能，已注释
