...
class InitialProjector(nn.Module):
    ...
    def svd_proj(self, adj):
        # 对于大型图，使用分块SVD计算
        max_svd_dim = min(512, args.latdim)  # 限制最大SVD维度
        q = min(max_svd_dim, adj.shape[0], adj.shape[1])
        
        # 设置GPU内存限制
        if hasattr(t.cuda, 'set_per_process_memory_fraction'):
            t.cuda.set_per_process_memory_fraction(args.gpu_memory_fraction)
        
        # 尝试分块SVD计算
        try:
            if not args.cache_proj:
                adj_gpu = adj.to(args.devices[0])
            else:
                adj_gpu = adj
            
            # 如果矩阵太大，使用分块计算
            if adj_gpu.shape[0] * adj_gpu.shape[1] > args.chunk_size * args.chunk_size:
                print(f"使用分块SVD计算，矩阵大小: {adj_gpu.shape}")
                svd_u, s, svd_v = self._chunked_svd(adj_gpu, q)
            else:
                svd_u, s, svd_v = t.svd_lowrank(adj_gpu, q=q, niter=args.niter)
            
            device = adj_gpu.device
            
        except RuntimeError as e:
            if "out of memory" in str(e) or "CUDA error" in str(e):
                print(f"GPU内存不足，尝试更小的分块...")
                t.cuda.empty_cache()
                ...
                try:
                    ...
                    # 使用更小的分块
                    smaller_chunk = args.chunk_size // 2
                    svd_u, s, svd_v = self._chunked_svd(adj_gpu, q, chunk_size=smaller_chunk)
                    device = adj_gpu.device
                    
                except RuntimeError as e2:
                    print(f"GPU仍然内存不足，回退到CPU...")
                    t.cuda.empty_cache()
                    adj_cpu = adj.cpu()
                    svd_u, s, svd_v = t.svd_lowrank(adj_cpu, q=q, niter=args.niter)
                    device = t.device('cpu')
            else:
                raise e
        ...
        if args.latdim > q:
            if device.type == 'cuda':
                svd_u = t.concat([svd_u, t.zeros([svd_u.shape[0], args.latdim-q]).to(device)], dim=1)
                svd_v = t.concat([svd_v, t.zeros([svd_v.shape[0], args.latdim-q]).to(device)], dim=1)
                s = t.concat([s, t.zeros(args.latdim-q).to(device)])
            else:
                svd_u = t.concat([svd_u, t.zeros([svd_u.shape[0], args.latdim-q])], dim=1)
                svd_v = t.concat([svd_v, t.zeros([svd_v.shape[0], args.latdim-q])], dim=1)
                s = t.concat([s, t.zeros(args.latdim-q)])
        ...
        # 如果不需要缓存且当前在GPU上，移回CPU
        if not args.cache_proj and device.type == 'cuda':
            projection = projection.cpu()
            
        return projection
    
    def _chunked_svd(self, adj, q, chunk_size=None):
        """简化的SVD计算，使用随机投影近似"""
        if chunk_size is None:
            chunk_size = args.chunk_size
        
        print(f"使用随机投影近似SVD: 矩阵大小 {adj.shape}, 目标维度 {q}")
        
        # 对于大型稀疏矩阵，使用随机投影近似SVD
        rows, cols = adj.shape
        
        # 生成随机投影矩阵
        omega = t.randn(cols, q, device=adj.device)
