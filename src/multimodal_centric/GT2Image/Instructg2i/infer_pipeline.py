...
class InstructG2IPipeline:
    def __init__(self, sd_pipe, sd_model_dir, image_encoder_path, proj_ckpt, gnn_ckpt, num_tokens, cross_attention_freq, neighbor_num, device,args):
        self.device = device
    ...
    @classmethod
    def from_pretrained(cls, sd_model_dir, neighbor_num, cache_dir='cache/', device='cpu' , args=None):
        ...
        return cls(sd_init,
                    sd_model_dir, #等于 args.model_dir
    ...
    def get_image_embeds(self, neighbor_image=None, clip_image_embeds=None):
        ...
        if neighbor_image is not None:
            ...
            for sample in neighbor_image:
                if sample == 'Empty':
                    clip_image_embeds += [torch.full((self.args.image_encoder_dim,),0,dtype=torch.float16).to(self.device)]
                else:
                    clip_image_embeds += [self.pretrained_image_embs[int(sample)].type(torch.float16)]
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if not isinstance(neighbor_mask, torch.LongTensor):
            # Convert neighbor_mask to a LongTensor
            neighbor_mask = torch.LongTensor(neighbor_mask)

        image_prompt_embeds, uncond_image_prompt_embeds = self.get_image_embeds(
            neighbor_image=neighbor_image, clip_image_embeds=clip_image_embeds # neighbor_image仅用于生成image_embedding
        )
