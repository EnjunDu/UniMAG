...
def parse_args():
    ...
    parser.add_argument(
        ...
        default='./config/train_Movies.json',
...
def main():
    ...
    image_proj_model = ImageProjModel(
        ...
        # clip_embeddings_dim=image_encoder.config.projection_dim,  # 替换为预训练好的image的dim
        clip_embeddings_dim=args.image_encoder_dim,  # 替换为预训练好的image的dim
    ...
    # Preprocessing the datasets.对数据集进行预处理
    ...
    # 获取训练数据集：
    train_dataset = GraphImageTextDataset(args.neighbor_num, os.path.join(args.train_data_dir,'train'),
                                          os.path.join(args.train_data_dir, f'{args.dataset_name}Images_extracted',f'{args.dataset_name}Images'), #MoviesImages_extracted /MoviesImages
    ...
    def collate_fn(examples):
        ...
        return {
            "neighbor_pixel_values": neighbor_pixel_values, # 邻节点的image
            "pixel_values": pixel_values, # 中心节点的image
            # 移除 input_ids
            # "input_ids": input_ids
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(graph_adapter):
                ...
                # 此处用到节点的pixel_values，但这是为了latent space
                with torch.no_grad():
                ...
                # 将文本嵌入设置为空
                with torch.no_grad():
                    encoder_hidden_states = torch.zeros_like(batch["input_ids"].to(accelerator.device), dtype=torch.float)
                ...
                #不载入文本embeding
                model_pred = graph_adapter(noisy_latents, timesteps, None, None, image_embeds, neighbor_mask)
