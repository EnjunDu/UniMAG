...
class AlignmentEvaluator(BaseEvaluator):
    ...
    def evaluate(self) -> Dict[str, float]:
        ...
        print("加载原始特征图...")
        # 访问 EmbeddingManager 内部的 StorageManager
        all_node_ids = self.embedding_manager._storage.load_node_ids(self.config['dataset']['name'])
        ...
        all_feature_maps = [sample_map.numpy() for _ in image_paths]
        ...
        with open(self.ground_truth_path, 'r', encoding='utf-8') as f:
            for line in tqdm(f, desc="评估对齐分数"):
                ...
                for grounding_pair in record["grounding"]:
                    ...
                    # RoIAlign requires (N, C, H, W)
                    if image_feature_map.ndim == 3:
                        image_feature_map = image_feature_map.unsqueeze(0)
                        
                    region_feature = roi_align(image_feature_map, [box_tensor], output_size=(1, 1), spatial_scale=1.0).flatten(1)
                    
                    # 确保维度匹配
                    phrase_dim = phrase_tensor.shape[1]
                    region_dim = region_feature.shape[1]
                    # 我们假设文本塔和图像塔的输入维度是相同的，因此如果它们不同，需要进行投影
                    if phrase_dim != region_dim:
                        # 这是一个简化的处理，实际应用中可能需要更复杂的对齐层
                        min_dim = min(phrase_dim, region_dim)
                        phrase_tensor = phrase_tensor[:, :min_dim]
                        region_feature = region_feature[:, :min_dim]
